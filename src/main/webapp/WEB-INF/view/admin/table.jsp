								...
								<tbody class="list">
									<c:forEach var="user" items="${list}" varStatus="loop">
										<tr>
											...
											<td><select name="role" id="role" onchange="changeRole('${user.username}')">
													<option value="USER">USER</option>
													<option value="ADMIN" ${user.role == "ADMIN" ? 'selected="selected"' : ''} >ADMIN</option>									
											</td>
											</select> 
											<td>
												<button type="button" class="btn btn-danger" id="deleteUser" onclick="deleteUser('${user.username}')">DELETE</button>
											</td>
										</tr>
									</c:forEach>
								</tbody>
							...
							<nav aria-label="...">
								<ul class="pagination justify-content-end mb-0">
									<li class="page-item disabled"><a class="page-link"
										href="/admin/table/1" tabindex="-1"> <i class="fas fa-angle-left"></i>
											<span class="sr-only">Previous</span>
									</a></li>
									<c:forEach varStatus="loop" begin="1" end="${numberPage}">
									<li class="page-item active"><a class="page-link" href="/admin/table/${loop.count}">${loop.count}</a>
									</li>
									</c:forEach>								
									<li class="page-item"><a class="page-link" href="/admin/table/${numberPage}"> <i
...
<script>
  function deleteUser(username){
    ...
    $.ajax({
                            ...
                            url: "http://localhost:8000/admin/table/delete/" + username,                     
                            // prevent jQuery from automatically transforming the data into a query string
                            success: function (response) {
                                window.location.href = "/admin/table/1";
                            },
