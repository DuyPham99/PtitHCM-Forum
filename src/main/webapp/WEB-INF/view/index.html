<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>PTIT-HCM: Diễn đàn trao đổi học tập!!</title>
	<!-- Import Boostrap css, js, font awesome here -->
	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css">       
	<link href="https://use.fontawesome.com/releases/v5.0.4/css/all.css" rel="stylesheet">  
	<link href="custom.css" rel="stylesheet">  
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js">
    </script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js">
    </script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.3/js/bootstrap.min.js"></script>
    <link href="../css/style.css" rel="stylesheet">
</head>
<body>

<!-- Navigation & Header -->
<header>
	<div class="container-fluid bg-white">
		<div class="row text-center border-bottom" >
			<div class="col text-right" >
				<a href="#"><img src="../resource/pictures/ptitLogo.jpg" height="70" class="logo" width="270" style="width: 30%;"></a>
				
			</div>
			<div class="col-6">
				<a href="#"><i class="fa fa-search"></i></a>
				<input class="header-search" size="40" type="text" placeholder="Tìm kiếm bài viết, tác giả,..." aria-label="Search">				
			</div>
			<div class="col align-self-center">
				<a href="#" class="text-header"> Đăng ký </a>
				<a href="#" class="ml-5 text-header"> Đăng nhập</a></div>
		  </div>
	</div>
	
</header>
<nav class="navbar navbar-expand-sm navbar-dark sticky-top bg-white header">
	<div class="container">		
		<ul class="nav col-md-12 nav-bar text-header" style="color: black;">
			<li class="nav-item ">
			  <a class="nav-link py-0" href="#">Trang chủ</a>
			</li>
			<li class="nav-item">
			  <a class="nav-link py-0" href="#">Hoạt động</a>
			</li>
			<li class="nav-item">
			  <a class="nav-link py-0" href="#">Học tập</a>
			</li>
			<li class="nav-item">
				<a class="nav-link py-0" href="#">Đội - Nhóm</a>
			</li>
			<li class="nav-item">
				<a class="nav-link py-0" href="#">Chuyện trò - tâm sự</a>
			</li>
			<li class="nav-item">
				<a class="nav-link py-0" href="#">Chia sẻ kinh nghiệm</a>
			</li>
			<li class="nav-item">
				<a class="nav-link py-0" href="#">Truyện cười</a>
			</li>
		  </ul>
  </div>
</div>
</nav>
  
<!--Start: Banner-->
<div class="container">
<div id="demo" class="carousel slide" data-ride="carousel">
	<!-- Indicators -->
	<ul class="carousel-indicators">
	  <li data-target="#demo" data-slide-to="0" class="active"></li>
	  <li data-target="#demo" data-slide-to="1"></li>
	  <li data-target="#demo" data-slide-to="2"></li>
	</ul>
	
	<!-- The slideshow -->
	<div class="carousel-inner">
	  <div class="carousel-item active">
		<img src="https://spiderum.com/assets/images/banner/cover-spiderum-not-login.jpg" alt="Los Angeles" width="1100" height="500">
		<div class="carousel-caption">
			<h3>Los Angeles</h3>
			<p>We had such a great time in LA!</p>
		</div>
	</div>
	  <div class="carousel-item">
		<img src="https://spiderum.com/assets/images/banner/cover-spiderum-not-login.jpg" alt="Chicago" width="1100" height="500">
		<div class="carousel-caption">
			<h3>Los Angeles</h3>
			<p>We had such a great time in LA!</p>
		</div>
	</div>
	  <div class="carousel-item">
		<img src="https://spiderum.com/assets/images/banner/cover-spiderum-not-login.jpg" alt="New York" width="1100" height="500">
		<div class="carousel-caption">
			<h3>Los Angeles</h3>
			<p>We had such a great time in LA!</p>
		</div>
	</div>
	</div>
	
	<!-- Left and right controls -->
	<a class="carousel-control-prev" href="#demo" data-slide="prev">
	  <span class="carousel-control-prev-icon"></span>
	</a>
	<a class="carousel-control-next" href="#demo" data-slide="next">
	  <span class="carousel-control-next-icon"></span>
	</a>
  </div>
</div>
<!--End: Banner-->
<!-- Start: Content -->
<div class="container">
	<p class="topic border-bottom">
		Hoạt động
		<span class=""></span><a href="#" class="more"> Xem thêm >></a>
		</span></a>
	</p>
	<div class="row row-cols-2" >
		<div class="col-md-6">
		<!--Start: Block-post-->
			<!--Start: Thumb-->
		 <div class="thumb">
			<a href="#">
				<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="552" height="270">
			</a>
		 </div>
		 <!--End: Thumb-->
		 <!--Start:Text-->
		 <div class="text border">
			<!--Start:Title-->
			<h5 class="title" >
				<a href="#"> Chủ nghĩa cộng sản </a>
			</h5>
			<!--End:Title-->
			<a href="" class="body">	
				<h6>Bài viết gốc đã được đăng tải vào 8/9/20 tại  Đây . Hãy like page Ăn sách nếu có thể để thể hiện sự ủng hộ cho tác giả, cám ơn các bạn.   Sự tôn thờ thanh xuân và tuổi trẻ ở Việt Nam...</h6>
			</a>
			<!--Start:author-->
			<a href="# ">
				<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
				<a href="#">Phạm Đức Duy</a>
			</a>
			<!--End : Author-->
		 </div>
		  <!--Start:Text-->
		 <!--End: Block-post-->
		</div>
		<div class="col-md-6">
			<div class="row row-cols-2">
				<div class="col-md-6" >
					<!--Start: Block-post-->
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
						</a>
					</div>
					<div class="text border">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				 <!--End: Block-post-->
				</div>
				<div class="col-md-6">
				<!--Start: Block-post-->
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
						</a>
					</div>
					<div class="text border">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				 <!--End: Block-post-->
				</div>
				</div>
				<div class="row row-cols-2">
					<div class="col-md-6 mt-2">
						<!--Start: Block-post-->
						<div class="img-thumbnail">
							<a href="#">
								<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
							</a>
						</div>
						<div class="text border">
							<h5 class="title" >
								<a href="#"> Chủ nghĩa cộng sản </a>
							</h5>
							<!--Start:author-->
							<a href="#">
								<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
								<a href="#">Phạm Đức Duy</a>
							</a>
						</div>
						<!--End: Block-post-->
					</div>
					<div class="col-md-6 mt-2">
						<!--Start: Block-post-->
						<div class="img-thumbnail">
							<a href="#">
								<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
							</a>
						</div>
						<div class="text border">
							<h5 class="title" >
								<a href="#"> Chủ nghĩa cộng sản </a>
							</h5>
							<!--Start:author-->
							<a href="#">
								<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
								<a href="#">Phạm Đức Duy</a>
							</a>
						</div>
					<!--End: Block-post-->
					</div>
				</div>
			</div>
			
		</div>
	</div>
</div>
<!-- End: Content -->
<!-- Start: Content -->	
<div class="container">
	<p class="topic border-bottom">
		Học tập
		<span class=""></span><a href="#" class="more"> Xem thêm >></a>
		</span></a>
	</p>
	<div class="row row-cols-2" >
		<div class="col-md-6">
		<!--Start: Block-post-->
			<!--Start: Thumb-->
		 <div class="thumb">
			<a href="#">
				<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="552" height="270">
			</a>
		 </div>
		 <!--End: Thumb-->
		 <!--Start:Text-->
		 <div class="text border">
			<!--Start:Title-->
			<h5 class="title" >
				<a href="#"> Chủ nghĩa cộng sản </a>
			</h5>
			<!--End:Title-->
			<a href="" class="body">	
				<h6>Bài viết gốc đã được đăng tải vào 8/9/20 tại  Đây . Hãy like page Ăn sách nếu có thể để thể hiện sự ủng hộ cho tác giả, cám ơn các bạn.   Sự tôn thờ thanh xuân và tuổi trẻ ở Việt Nam...</h6>
			</a>
			<!--Start:author-->
			<a href="#">
				<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
				<a href="#">Phạm Đức Duy</a>
			</a>
			<!--End : Author-->
		 </div>
		  <!--Start:Text-->
		 <!--End: Block-post-->
		</div>
		<div class="col-md-6">
			<div class="row row-cols-2">
				<div class="col-md-6" >
					<!--Start: Block-post-->
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
						</a>
					</div>
					<div class="text border">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				 <!--End: Block-post-->
				</div>
				<div class="col-md-6">
				<!--Start: Block-post-->
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
						</a>
					</div>
					<div class="text border">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				 <!--End: Block-post-->
				</div>
				</div>
				<div class="row row-cols-2">
					<div class="col-md-6 mt-2">
						<!--Start: Block-post-->
						<div class="img-thumbnail">
							<a href="#">
								<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
							</a>
						</div>
						<div class="text border">
							<h5 class="title" >
								<a href="#"> Chủ nghĩa cộng sản </a>
							</h5>
							<!--Start:author-->
							<a href="#">
								<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
								<a href="#">Phạm Đức Duy</a>
							</a>
						</div>
						<!--End: Block-post-->
					</div>
					<div class="col-md-6 mt-2">
						<!--Start: Block-post-->
						<div class="img-thumbnail">
							<a href="#">
								<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="245" height="130">
							</a>
						</div>
						<div class="text border">
							<h5 class="title" >
								<a href="#"> Chủ nghĩa cộng sản </a>
							</h5>
							<!--Start:author-->
							<a href="#">
								<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
								<a href="#">Phạm Đức Duy</a>
							</a>
						</div>
						<!--End: Block-post-->
					</div>

				</div>
			</div>
			
		</div>
	</div>
</div>
<!-- End: Content -->
<!-- Start: Content -->
<div class="container">
	<p class="topic border-bottom">
		Đội - Nhóm
		<span class=""></span><a href="#" class="more"> Xem thêm >></a>
		</span></a>
	</p>
	<!--Start: Large -->
	<div class="row border study" >
		<div class="col-md-6 blockpost">
			<img style="" src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="910" height="270">
		</div>
		<div class="col-md-6 blockpost">
			<div class="text">
				<h5 class="title">
					<a href="">Các vấn đề của xã hội ngày nay</a>
				</h5>
				<a class="body" href="#">
					<h6>Không có vấn đề gì đang sợ cả nha....</h6>
				</a>
				<div class="author">
					<a href="#">
						<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
					</a>
					<a href="#" style="">Phạm Đức Duy</a>
				</div>
			</div>
		</div>
	</div>
	<!-- End: Large -->
	<div class="row mt-3">
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="row mt-3">
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!-- End: Content -->
<!-- Start: Content -->
<div class="container">
	<p class="topic border-bottom">
		Chuyện trò - tâm sự
		<span class=""></span><a href="#" class="more"> Xem thêm >></a>
		</span></a>
	</p>
	<div class="row justify-content-center mt-3">
		<div class="col-md-3 col-md-offset-1">
			<div class="background">
				<div class="text">
					<div style="position: absolute;bottom: 40px;margin-top: 0">
						<h5 class="title">
							<a href="" style="color: aliceblue;">Các vấn đề của xã hội ngày nay</a>
						</h5>
					</div>
					
					<div class="author">
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
						</a>
						<a href="#" style="color: aliceblue;">Phạm Đức Duy</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-3">
			<div class="background">
				<div class="text">
					<div style="position: absolute;bottom: 40px;margin-top: 0">
						<h5 class="title">
							<a href="" style="color: aliceblue;">Các vấn đề của xã hội ngày nay</a>
						</h5>
					</div>
					
					<div class="author">
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
						</a>
						<a href="#" style="color: aliceblue;">Phạm Đức Duy</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-3">
			<div class="background">
				<div class="text">
					<div style="position: absolute;bottom: 40px;margin-top: 0">
						<h5 class="title">
							<a href="" style="color: aliceblue;">Các vấn đề của xã hội ngày nay</a>
						</h5>
					</div>
					
					<div class="author">
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
						</a>
						<a href="#" style="color: aliceblue;">Phạm Đức Duy</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-3">
			<a href="#">
				<div class="background">
					<div class="text">
						<div style="position: absolute;bottom: 40px;margin-top: 0">
							<h5 class="title">
								<a href="" style="color: aliceblue;">Các vấn đề của xã hội ngày nay</a>
							</h5>
						</div>
						
						<div class="author">
							<a href="#">
								<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							</a>
							<a href="#" style="color: aliceblue;">Phạm Đức Duy</a>
						</div>
					</div>
				</div>
			</a>
		</div>
	</div>
</div>
<!--End: Content-->
<!--Start: Kinh nghiệm-->
<div class="container">
	<p class="topic border-bottom">
		Chia sẻ kinh nghiệm
		<span class=""></span><a href="#" class="more"> Xem thêm >></a>
		</span></a>
	</p>
	<div class="row mt-3">
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div class="row mt-3">
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-6">
			<div class="row">
				<div class="col-md-6">
					<div class="img-thumbnail">
						<a href="#">
							<img src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?ixlib=rb-1.2.1&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=1080&fit=max" width="100" height="200">
						</a>
					</div>
				</div>
				<div class="col-md-6">
					<div class="text">
						<h5 class="title" >
							<a href="#"> Chủ nghĩa cộng sản </a>
						</h5>
						<!--Start:author-->
						<a href="#">
							<img src="data:image/png;base64," alt="Icon Account" style="float:left;width:20px;height:20px;margin-top: 5px; margin-right: 5px; margin-left: 3px;">
							<a href="#">Phạm Đức Duy</a>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<!--End: Chia sẻ kn-->
<!-- Start: Truyện cười-->
<div class="container">
	<p class="topic border-bottom">
		Truyện cười
		<span class=""></span><a href="#" class="more"> Xem thêm >></a>
		</span></a>
	</p>
	<div class="row">
	   <div class="col-4">
		  <nav id="myScrollspy">
			 <ul class="nav nav-pills flex-column">
				<li class="nav-item"><a class="nav-link" href="#content-javascript">Javascript</a></li>
				<li class="nav-item"><a class="nav-link" href="#content-css">CSS</a></li>
				<li class="nav-item"><a class="nav-link" href="#content-bootstrap">Bootstrap</a></li>
			 </ul>
		  </nav>
	   </div>
	   <div class="col-8">
		  <div data-spy="scroll" data-target="#myScrollspy" data-offset="10"
				style="height:200px;overflow-y: scroll;padding:5px; border: 1px solid #ccc;">
			 <h4 id="content-javascript">Javascript</h4>
			 <p>
			   JavaScript is a cross-platform, object-oriented scripting language used to make webpages interactive
				(e.x. having complex animations, clickable buttons, popup menus, etc.)...
			 </p>
			 <h4 id="content-css">CSS</h4>
			 <p>
			   CSS is the language for describing the presentation of Web pages, including colors,
				layout, and fonts. It allows one to adapt the presentation to different types of devices,
				such as large screens, small screens, or printers...
			 </p>
			 <h4 id="content-bootstrap">Bootstrap</h4>
			 <p>
			   CSS is the language for describing the presentation of Web pages, including colors,
				layout, and fonts. It allows one to adapt the presentation to different types of devices,
				such as large screens, small screens, or printers. CSS is independent of HTML
				and can be used with any XML-based markup language...
			 </p>
		  </div>
	   </div>
	 </div>	 
</div>
<!-- End: Truyện cười-->
<hr>
</body>
<div class="container" style="font-size: 10px;">
	<p>Học viện Công nghệ Bưu chính - TP.HCM</p>
	<p>Địa chỉ 1: Quận 1</p>
	<p>Địa chỉ 2: 97 Man Thiện, phường Tăng Nhơn Phú A, quận 9, TP.Hồ Chí Minh</p>
</div>

</html>